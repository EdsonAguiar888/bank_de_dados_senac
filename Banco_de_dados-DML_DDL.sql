...
--- Exemplo 1: Alterar o status de um Pedido Vamos alterar o status do Pedido 2 de 'Em Preparo' para 'Finalizado'.

UPDATE Pedidos
SET status = 'Finalizado'
WHERE id_pedido = 2;
--- Exemplo 2: Alterar o Preço de um Produto Vamos aumentar o preço do 'Espresso' para R$ 6,00.

UPDATE Produtos
SET preco = 6.00
WHERE nome_produto = 'Espresso';
--- Exemplo 3: Adicionar Email a um Cliente (preenchendo um campo nulo) O 'Carlos Lima' estava sem email. Vamos adicioná-lo.
...
--- Exemplo 1: Excluir um Produto Vamos excluir o produto 'Cheesecake' (id_produto 4).

-- Primeiro, é crucial garantir que nenhum item de pedido dependa deste produto, -- ou a exclusão falhará devido à Chave Estrangeira (FOREIGN KEY). -- Como a cheesecake não está nos pedidos de exemplo, podemos excluir diretamente:

    
DELETE FROM Produtos
WHERE nome_produto = 'Cheesecake';
--- Exemplo 2: Excluir um Cliente e Seus Pedidos (Necessidade de Cuidado) Se você tentar excluir um cliente que tenha pedidos registrados, o banco de dados geralmente bloqueará a operação (se as restrições de FOREIGN KEY estiverem ativas) para evitar pedidos "órfãos". Você precisa excluir os dados dependentes primeiro.

--- Excluir Itens do Pedido Primeiro, exclua os itens de pedido que pertencem aos pedidos do cliente. -- Exclui os itens do Pedido 1 (pertencente à Ana - id_cliente 1)
DELETE FROM Itens_Pedido
WHERE id_pedido IN (SELECT id_pedido FROM Pedidos WHERE id_cliente = 1);
--- Excluir o Pedido Em seguida, exclua o pedido principal. -- Exclui o Pedido 1
DELETE FROM Pedidos
WHERE id_cliente = 1;
--- Excluir o Cliente Finalmente, você pode excluir o cliente.
